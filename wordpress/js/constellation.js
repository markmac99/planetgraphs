...
//
function getConstellation(ra, dec, epoch)
{
...
function parse_coordinates(str)
{
  ...
  for (nra = 0; nra < exploded.length; nra++)
      if (exploded[nra].length > 0 &&
          (exploded[nra].charAt(0) == '+' || exploded[nra].charAt(0) == '-'))
          break;

  ndec = exploded.length - nra;

  if (nra < 1 || nra > 3 || ndec < 1 || ndec > 3)
     return [];

  rah = parseFloat(exploded[0]);
  if (nra >= 2)
     ram = parseFloat(exploded[1]);
  if (nra >= 3)
     ras = parseFloat(exploded[2]);

  decd = parseFloat(exploded[nra]);
  if (ndec >= 2)
     decm = parseFloat(exploded[nra + 1]);
  if (ndec >= 3)
     decs = parseFloat(exploded[nra + 2]);

  if (isNaN(rah) || isNaN(ram) || isNaN(ras) || isNaN(decd) || isNaN(decm) || isNaN(decs))
     return [];

  if (rah < 0.0 || rah >= 24.0 || ram < 0.0 || ram >= 60.0 || ras < 0.0 || ras >= 60.0)
     return [];

  if (decd < -90.0 || decd > 90.0 || decm < 0.0 || decm >= 60.0 || decs < 0.0 || decs >= 60.0)
     return [];

  if (decd < 0.0 ||
      (decd == 0.0 && exploded[nra].length > 0 &&
  ...
  return [rah + ram / 60.0 + ras / 3600.0, decd + decm / 60.0 + decs / 3600.0];
}
...
function main(str)
{
  ...
  coordlist = parse_coordinates(str);
